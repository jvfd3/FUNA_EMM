...
def prepare_beam_and_candidate_result_set(candidate_result_set=None, cq_satisfied=None, sel_params=None, general_params=None):
    ...
    if len(cq_satisfied) > 0:
        ...
        candidate_queue, n_redun_descs = apply_dbs_wcs(cq_sorted=cq_sorted, sel_params=sel_params, general_params=general_params,
                                                       stop_number_dbs=4*sel_params['w'], stop_number_wcs=sel_params['w'], wcs_gamma=sel_params['gamma'])
        ...
        candidate_result_set.append(
            selected_for_result_list)  # creates a nested list
        # unlist alle elements
        rs_candidates = [
            item for sublist in candidate_result_set for item in sublist]
...
def prepare_result_set(candidate_result_set=None, general_params=None, sel_params=None):
    ...
    result_set_ordered = sort_list_on_quality(
        list_unsorted=candidate_result_set)

    # here 'w' is replaced with 'q'
    result_set, rs_n_redun_descs = apply_dbs_wcs(cq_sorted=result_set_ordered, sel_params=sel_params, general_params=general_params,
                                                 stop_number_dbs=4*sel_params['q'], stop_number_wcs=sel_params['q'], wcs_gamma=sel_params['gamma'])
...
def sort_list_on_quality(list_unsorted=None):
    ...
    list_sorted = sorted(
        list_unsorted, key=lambda i: i['qualities']['varphi'], reverse=True)
...
def apply_dbs_wcs(cq_sorted=None, sel_params=None, general_params=None, stop_number_dbs=None, stop_number_wcs=None, wcs_gamma=None):
    ...
    if sel_params['dbs']:
        ...
        candidates, n_redun_descs = dbs.remove_redundant_descriptions(
            descs=cq_sorted, sel_params=sel_params, stop_number_dbs=stop_number_dbs)
    else:
    ...
    if sel_params['wcs']:
        ...
        candidate_queue = cbs.select_using_weighted_coverage(
            candidates=candidates, sel_params=sel_params, general_params=general_params, stop_number_wcs=stop_number_wcs,  wcs_gamma=wcs_gamma)
    else:
