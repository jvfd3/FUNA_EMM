...
def result_set_to_df(result_set=None):
    ...
    if len(result_set) > 0:
        ...
        descs_result_set_dict = {sg: desc['description']
                                 for sg, desc in enumerate(result_set)}
        descs_pd = pd.DataFrame.from_dict(
            descs_result_set_dict, orient='index')
...
def obtain_subgroup_results(result_set=None):
    ...
    subgroup_results = [(desc['adds']['literal_order'],
                         ...
                         # will remove these later when saving the result set
                         desc['adds']['idxIDs']['idx_id'],
                         # will remove these later when saving the result set
                         desc['adds']['idxIDs']['idx_sg'])
    ...
    subgroup_results_pd = pd.concat(
        (subgroup_results_pd, dfall.T.reset_index(drop=True)), axis=1)
