...
def update_output_file(excel_file_name=None, sheet_names=None, result_sum=None, info_sum=None):
    ...
    if info_sum is not None:
        ...
        dfnew = pd.concat((df, pd.Series(info_sum).to_frame(
...
def save_and_store_result(simulation_result=None, extra_info=None, output_to_path=None, excel_file_name=None, sheet_names=None):
    ...
    if result_emm is not None:
        ...
        store_result_emm.to_csv(
            output_to_path + str(list(sel_params.values())) + '.txt', sep='\t', index=False)
    ...
    if distribution is not None:
        distribution_params = prr.obtain_summary_values_dfd(
            distribution=distribution)
        result_sum.update(distribution_params)
        result_sum.update(prr.calculate_number_rejected(
            result_emm=result_emm, distribution_params=distribution_params))
    ...
    update_output_file(excel_file_name=excel_file_name,
                       result_sum=result_sum, info_sum=info_sum, sheet_names=sheet_names)
