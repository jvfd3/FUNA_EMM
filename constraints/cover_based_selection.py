...
def select_using_weighted_coverage(candidates=None, sel_params=None, general_params=None, stop_number_wcs=None, wcs_gamma=None):
    ...
    if len(candidates) > stop_number_wcs:
        ...
        temp_allIDs = pd.DataFrame({data_idx_col: allIDs, 'count': np.zeros(len(allIDs))})
        ...
        while i < stop_number_wcs:
            ...
            temp_allIDs.loc[temp_allIDs[data_idx_col].isin(selidxIDs),'count'] += 1
            ...
            for candidate in left_over_candidates:
                ...
                all_weights = np.power(wcs_gamma, temp_allIDs.loc[temp_allIDs[data_idx_col].isin(selidxIDs),'count'].values)
